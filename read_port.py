...
def create_table():
    cursor.execute("CREATE TABLE IF NOT EXISTS dataToPlot(height1 REAL, time1 REAL)")  #dataToPlot is the name of the table that we create
...
def dynamic_data_entry():
    ...
    for i in range (3, 10):
        cursor.execute("INSERT INTO dataToPlot (height1, time1) VALUES (?, ?)", (int(hght), int(i)))   #for inserting variables
        connection.commit()
        time.sleep(2)
    #connection.commit()
